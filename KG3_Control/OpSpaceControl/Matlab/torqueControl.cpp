...
bool example_cyclic_torque_control(k_api::Base::BaseClient* base, k_api::BaseCyclic::BaseCyclicClient* base_cyclic, k_api::ActuatorConfig::ActuatorConfigClient* actuator_config)
{
    ...
    SystemKG3 systemKG3; // Create an instance of SystemKG3
    Eigen::VectorXd Fext(6); // External forces (example, replace with actual values)
    Fext << 0, 0, 0, 0, 0, 0; // Replace with actual external forces if needed
    ...
    try
    {
        ...
        // Set actuators 2, 4, and 6 to torque mode
        std::vector<int> torque_actuators = {2, 4, 6};  // Device IDs for the actuators
        for(const auto& actuator_id : torque_actuators) {
        ...
        while (timer_count < (TIME_DURATION * 1000))
        {
            ...
            if (now - last > 1000)
            {
                ...
                for (unsigned int i = 0; i < actuator_count; ++i) {
                    if (i == 0){
                        q(i) = 360.0 - base_feedback.actuators(i).position(); // Adjust for Kinova's coordinate system
                        ...
                        q(i) = 160.0 - base_feedback.actuators(i).position(); // Adjust for Kinova's coordinate system
                        dqdt(i) = -base_feedback.actuators(i).velocity(); // Adjust for Kinova's coordinate system
                    }
                    else{
                ...
                if (timer_count % 500 == 0) {
                    std::cout << "Joint Positions (rad) at t = " << timer_count << "ms: " << q.transpose() << std::endl;
                    std::cout << "Joint Velocities (rad/s) at t = " << timer_count << "ms: " << dqdt.transpose() << std::endl;
                }
                
                x << dqdt, q; // Concatenate dqdt and q into a single vector
                u = systemKG3.computeInput(x, Fext); // Call computeInput method from SystemKG3
                
                // Set torque commands for actuators 2, 4, and 6
                std::vector<std::pair<int, int>> torque_mapping = {
                    {1, 2}, // Index 1 of u maps to actuator 2
                    {3, 4}, // Index 3 of u maps to actuator 4
                    {5, 6}  // Index 5 of u maps to actuator 6
                };

                for (const auto& [u_idx, actuator_id] : torque_mapping) {
                    // Set position to current position to avoid following error
                    base_command.mutable_actuators(actuator_id - 1)->set_position(
                        base_feedback.actuators(actuator_id - 1).position());
                    // Set computed torque command
                    base_command.mutable_actuators(actuator_id - 1)->set_torque_joint(u[u_idx]);
                }
        ...
        // Set all torque-controlled actuators back to position mode
        control_mode_message.set_control_mode(k_api::ActuatorConfig::ControlMode::POSITION);
        for(const auto& actuator_id : torque_actuators) {  // Using the same vector we defined earlier
            actuator_config->SetControlMode(control_mode_message, actuator_id);
        }
