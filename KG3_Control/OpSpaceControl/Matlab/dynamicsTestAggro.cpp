...
int main(int argc, char **argv)
{
    ...
    //---------------------------------------------------------
    bool success = true;
    SystemKG3FullInputMoreAggressive systemKG3; // Create an instance of SystemKG3
        ...
        while (timer_count < (TIME_DURATION * 1000)) // Loop runs for TIME_DURATION seconds at ~1kHz
        {
            ...
            if (now - last >= 1000) // Check if 1000 microseconds (1ms) have passed
            {
                try
                {
                    ...
                    // J: 6×n Eigen::Matrix<double,6,n>
                    // tau_ext: Eigen::Matrix<double,n,1>
                    ...
                    for (unsigned int i = 0; i < 6; ++i) { // Loop through indices 0 to 5 (Actuators 1 to 6)
                        // Apply scaling factor if needed (example: 1.0, adjust per actuator)
                        double scale_factor = 1.0; // Adjust if necessary, e.g., based on previous factors
                    ...
                    // 3. Increment Frame ID (already exists)
