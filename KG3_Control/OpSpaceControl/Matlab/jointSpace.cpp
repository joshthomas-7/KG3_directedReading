...
// ============================================================================
//  Trajectory Data Structures and Loader
// ============================================================================
...
struct TrajectoryPoint {
    ...
    Eigen::VectorXd qddot;  // Joint accelerations (7)
    
    TrajectoryPoint() : q(7), qdot(7), qddot(7) {}
};
...
public:
    ...
    bool loadFromCSV(const std::string& filename, bool has_header = true) {
        ...
        while (std::getline(file, line)) {
            ...
            // Check if we have the right number of columns (1 + 7 + 7 + 7 = 22)
            if (row.size() != 22) {
                std::cerr << "Warning: Skipping malformed line (expected 22 columns, got " 
                          << row.size() << ")" << std::endl;
                continue;
            }
            ...
            // Joint positions (columns 1-7)
            for (int i = 0; i < 7; ++i) {
                point.q(i) = row[1 + i];
            }
            
            // Joint velocities (columns 8-14)
            for (int i = 0; i < 7; ++i) {
                point.qdot(i) = row[8 + i];
            }
            
            // Joint accelerations (columns 15-21)
            for (int i = 0; i < 7; ++i) {
                point.qddot(i) = row[15 + i];
            }
        ...
        file.close();
        std::cout << "Loaded " << trajectory.size() << " trajectory points" << std::endl;
    ...
    const TrajectoryPoint& getPoint(size_t index) const {
        if (index >= trajectory.size()) {
            return trajectory.back(); // Return last point if out of bounds
        }
        return trajectory[index];
    }
    
    // Get the sample time (dt) from the trajectory
    double getSampleTime() const {
        if (trajectory.size() < 2) {
            return 0.001; // Default to 1ms if cannot determine
        }
...
int main(int argc, char **argv)
{
    ...
    //---------------------------------------------------------
    TrajectoryLoader trajectory;
    std::string csv_filename = "./kg3_trajectory.csv";
    ...
    // Override TIME_DURATION to match trajectory duration (add a small buffer)
    TIME_DURATION = static_cast<float>(trajectory.getDuration() + 0.5);
    std::cout << "Setting control duration to " << TIME_DURATION << " seconds" << std::endl;
    ...
    // Joint space trajectory - now loaded from CSV
    Eigen::VectorXd qstar(7);        // Desired joint configuration
    
    // Initialize qstar with the first point from the trajectory
    TrajectoryPoint initial_point = trajectory.getPoint(0);
    qstar = initial_point.q;
    
    std::cout << "Initial desired joint configuration: " << qstar.transpose() << std::endl;

    double KP_aggro = 1500.0;                // Tuneable proportional gain for joint space control
    double KD_aggro = 10.0;                // Tuneable derivative gain for joint space control
    ...
    std::vector<Eigen::VectorXd>   qstar_log; qstar_log.reserve(maxIter); // NEW: Log desired trajectory
    ...
    try
    {
        ...
        // Track trajectory index for discrete samples
        size_t trajectory_index = 0;
        double trajectory_sample_time = trajectory.getSampleTime();
        int samples_per_trajectory_point = static_cast<int>(trajectory_sample_time / 0.001); // Convert trajectory dt to number of 1ms control cycles
        
        std::cout << "Control loop runs at 1 kHz (1ms)" << std::endl;
        std::cout << "Trajectory sample time: " << trajectory_sample_time << " s" << std::endl;
        std::cout << "Will update qstar every " << samples_per_trajectory_point << " control cycles" << std::endl;
        ...
        while (timer_count < (TIME_DURATION * 1000)) // Loop runs for TIME_DURATION seconds at ~1kHz
        {
            ...
            if (now - last >= 1000) // Check if 1000 microseconds (1ms) have passed
            {
                try
                {
                    ...
                    // ============================================================
                    // Update desired joint configuration at trajectory sample intervals
                    // ============================================================
                    // Check if it's time to switch to the next trajectory point
                    if (timer_count % samples_per_trajectory_point == 0 && trajectory_index < trajectory.size()) {
                        TrajectoryPoint desired_point = trajectory.getPoint(trajectory_index);
                        qstar = desired_point.q;
                        // You can also use desired_point.qdot and desired_point.qddot in your controller
                        trajectory_index++;
                    }
                    // ============================================================

                    u = systemKG3.jointSpaceControl(x, qstar, KP_aggro, KD_aggro);

                    
                    if (timer_count % 250 == 0) { // Adjust print frequency as needed
                        std::cout << "t = " << timer_count << "ms, trajectory index = " << trajectory_index << "/" << trajectory.size() << std::endl;
                        std::cout << "Calculated Dynamics Torques (u) at t = " << timer_count << "ms: " << u.transpose() << std::endl;
                        std::cout << "Current joint config (q): " << q.transpose() << std::endl;
                        std::cout << "Desired joint config (qstar): " << qstar.transpose() << std::endl;
                        std::cout << "Current joint velocities (dqdt): " << dqdt.transpose() << std::endl;
                    }
                    
                    // Limit torques 
                    for (unsigned int i = 0; i < actuator_count; i++) {
                        // u[i] = std::max(-5.0, std::min(5.0, u[i]));
                        Eigen::VectorXd g = systemKG3.gq;
                        if (u[i] > 30.0 || u[i] < -30.0) {
                            std::cout << "Large torque: " << u[i] << std::endl;
                    ...
                    qstar_log.push_back(qstar); // NEW: Log desired trajectory
...
if (!csv.is_open()) {
    ...
    // NEW: Updated header to include desired trajectory
    csv << "iter";
    for (int j = 0; j < 7; ++j) csv << ",u"    << (j+1);
    for (int j = 0; j < 7; ++j) csv << ",q"    << (j+1);
    for (int j = 0; j < 7; ++j) csv << ",dq"   << (j+1);
    for (int j = 0; j < 7; ++j) csv << ",qstar" << (j+1); // NEW
    ...
    for (int i = 0; i < N; ++i) {
        ...
        // NEW: write all 7 entries of qstar_log[i]
        for (int j = 0; j < 7; ++j) {
            csv << "," << qstar_log[i](j);
        }
