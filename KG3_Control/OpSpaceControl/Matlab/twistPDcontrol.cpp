...
extern "C" void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    try {
        ...
        // Call the C++ function
        KinovaApiWrapper::RobotState state = api.twistPDcontrol(xDesired, xdotDesired, pedalState);

        // Create outputs based on number of requested output arguments
        if (nlhs >= 1) { // Haptic wrench [6x1]
            plhs[0] = mxCreateDoubleMatrix(6, 1, mxREAL);
            double* out = mxGetPr(plhs[0]);
            for (size_t i = 0; i < 6; ++i) {
                out[i] = state.haptic_wrench[i];
            }
        }

        if (nlhs >= 2) { // Current position [3x1]
        ...
        if (nlhs >= 3) { // Current RPY angles [3x1]
        ...
        if (nlhs >= 4) { // Current linear velocity [3x1]
            plhs[3] = mxCreateDoubleMatrix(3, 1, mxREAL);
            double* out = mxGetPr(plhs[3]);
            for (size_t i = 0; i < 3; ++i) {
                out[i] = state.current_vel[i];
            }
        }

        if (nlhs >= 5) { // Current angular velocity [3x1]
        ...
        if (nlhs >= 6) { // Twist command [6x1]
            plhs[5] = mxCreateDoubleMatrix(6, 1, mxREAL);
            double* out = mxGetPr(plhs[5]);
            for (size_t i = 0; i < 6; ++i) {
