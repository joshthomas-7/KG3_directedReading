...
int main(int argc, char **argv)
{
    ...
    Eigen::VectorXd qstar1(7);        // 1st desired joint configuration
    Eigen::VectorXd qstar2(7);        // 2nd desired joint configuration
    qstar1 << 0.50, 0.2618, 3.14161, 4, 5, 0.30, 1.57082;
    qstar2 << 0.50, 0.2618, 3.14161, 4, 6, 1.00, 1.57082;

    // Start with qstar1
    qstar = qstar1;

    bool swtiched_to_qstar2 = false;    // Boolean used to check if we have switched to the second desired joint config
    double proximity;                   // Proximity to the desired joint configuration
    double threshold = 0.125;             // Threshold that the proximity must be less than to switch to the second joint configuration
    double aggro = 90.0;                // Tuneable proportional controller gain for gravity compensation (KP_grav = aggro*KP_grav)
        ...
        while (timer_count < (TIME_DURATION * 1000)) // Loop runs for TIME_DURATION seconds at ~1kHz
        {
            ...
            if (now - last >= 1000) // Check if 1000 microseconds (1ms) have passed
            {
                try
                {
                    ...
                    if (!swtiched_to_qstar2) {
                        // Check proximity to first joint configuration, to check if to switch to qstar2
                        proximity = (q - qstar1).norm();
                        // std::cout << "proximity: " << proximity << std::endl;
                        if (proximity < threshold) {
                            qstar = qstar2;
                            swtiched_to_qstar2 = true;
                        }
                    }


                    u = systemKG3.gravityCompensation(x, qstar, aggro);
