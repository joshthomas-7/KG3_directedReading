...
public:
    ...
    // Joint damping
    const double d1 = 0.1;    // [N.m.s/rad]
    const double d2 = 0.1;    // [N.m.s/rad]
    const double d3 = 0.1;    // [N.s/m]
    const double d4 = 0.1;    // [N.m.s/rad]
    const double d5 = 0.1;    // [N.m.s/rad]
    const double d6 = 0.1;    // [N.m.s/rad]
    const double d7 = 0.1;    // [N.s/m]
    const double d8 = 0.1;    // [N.m.s/rad]
    ...
    // EE RPY angles (for use in the angle = atan2(y,x) function)
    double xtheta;
    double ytheta;
    double xphi;
    double yphi;
    ...
    // Public methods
    Eigen::VectorXd computeInput(const Eigen::VectorXd &x, const Eigen::VectorXd &Fext);
