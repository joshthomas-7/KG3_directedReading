...
public:
    ...
    // Link mass [kg]
    const double m1 = 1.697;  
    const double m2 = 1.377;  
    const double m3 = 1.1636; 
    const double m4 = 1.1636;
    const double m5 = 0.930;  
    const double m6 = 0.678;  
    const double m7 = 0.678;  
    const double m8 = 1.425;   
    ...
    // HA: Analytical Hessian
    std::vector<Eigen::Matrix<double,6,7>> HA; // Each element: 6x7

    // Dynamic matrices (dimensions follow MATLAB code)
    ...
    // Integral error
    Eigen::VectorXd integralError = Eigen::VectorXd::Zero(6); // integral error for integral action controller
    Eigen::VectorXd integral_max = Eigen::VectorXd::Zero(6); // integral error for integral action controller

    // Joint space controller matricies (7x7)
    Eigen::Matrix<double, 7, 7> KP_grav;
    Eigen::Matrix<double, 7, 7> KD_grav;
    ...
    // Additional 4x4 transformation matrices 
    Eigen::Matrix4d A0c2;
    Eigen::Matrix4d A0c3;
    Eigen::Matrix4d A0c4;
    Eigen::Matrix4d A0c5;
    ...
    Eigen::VectorXd opSpaceControl(const Eigen::VectorXd &x, const Vec6d &xstar_in, const double KP_aggro, const double KD_aggro);
    Eigen::VectorXd opSpaceControl_IA(const Eigen::VectorXd &x, const Vec6d &xestar_in, const double KP_aggro, const double KD_aggro, const double KI_aggro);
    Eigen::VectorXd jointSpaceControl(const Eigen::VectorXd &x, const Vec6d &qstar_in, const double KP_aggro, const double KD_aggro);
    Eigen::VectorXd gravityCompensation(const Eigen::VectorXd &x, const Eigen::VectorXd &qstar_in, const double aggro);
    ...
    // Angle wrapping methods
    double wrapTo2Pi(double angle);
    double computeAngularError(double target, double current);
    Eigen::VectorXd wrapTo2Pi(const Eigen::VectorXd& angles);
    Eigen::VectorXd computeAngularError(const Eigen::VectorXd& target, const Eigen::VectorXd& current);
